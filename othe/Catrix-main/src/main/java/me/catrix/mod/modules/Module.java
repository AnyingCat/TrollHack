package me.catrix.mod.modules;

import me.catrix.Catrix;
import me.catrix.core.impl.CommandManager;
import me.catrix.core.impl.ModuleManager;
import me.catrix.mod.Mod;
import me.catrix.mod.modules.impl.client.*;
import me.catrix.mod.modules.impl.hud.*;
...
public abstract class Module extends Mod {
    ...
    private boolean listHide() {
        return this instanceof Colors || this instanceof BaritoneModule || this instanceof AntiCheat || this instanceof ClientSetting || this instanceof ServerApply || this instanceof HUD || this instanceof ModuleList || this instanceof ArmorHud || this instanceof ItemsCountHud || this instanceof DynamicIsland || this instanceof PotionHud || this instanceof TargetHud || this instanceof SelfHud || this instanceof  InventoryHud || this instanceof KeyDisplayHud || this instanceof Sound || this instanceof Capes;
    }
    ...
    public void enable() {
        ...
        if (!nullCheck() && drawnSetting.getValue() && ClientSetting.INSTANCE.toggle.getValue()) {
            ...
            if (DynamicIsland.INSTANCE.isOn()) {
                DynamicIsland.INSTANCE.ModuleEnableNotification(this.getDisplayName());
            } else {
                sendMessageEnable(id);
            }
        }
        this.state = true;
        Catrix.EVENT_BUS.subscribe(this);
        Catrix.SOUND.playEnable();
        this.onToggle();
        try {
            this.onEnable();
        } catch (Exception e) {
    ...
    public void disable() {
        ...
        if (!nullCheck() && drawnSetting.getValue() && ClientSetting.INSTANCE.toggle.getValue()) {
            int id = ClientSetting.INSTANCE.onlyOne.getValue() ? -1 : hashCode();
            if (DynamicIsland.INSTANCE.isOn()) {
                DynamicIsland.INSTANCE.ModuleDisableNotification(this.getDisplayName());
            } else {
                sendMessageOnDisable(id);
            }
        }
        this.state = false;
        Catrix.EVENT_BUS.unsubscribe(this);
        Catrix.SOUND.playDisable();
        this.onToggle();
        this.onDisable();
    }

    private void sendMessageEnable(int id) {
    ...
    private void sendMessageOnDisable(int id) {
        switch (ClientSetting.INSTANCE.messageStyle.getValue()) {
