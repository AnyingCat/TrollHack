...
import me.catrix.asm.accessors.ILivingEntity;
...
public class KillAura extends Module {
    ...
    private final EnumSetting<TargetESP> mode = add(new EnumSetting<>("TargetESP", TargetESP.CaptureMark, () -> page.getValue() == Page.Render));
    private final ColorSetting color = add(new ColorSetting("Color", new Color(0x6CFFFFFF,true), () -> page.getValue() == Page.Render));
    ...
    public static void doRender(MatrixStack matrixStack, float partialTicks, Entity entity, Color color, TargetESP mode) {
        switch (mode) {
            ...
            case Jello -> TargetESPUtil.drawJello(matrixStack, entity, color);
            case Ghosts -> TargetESPUtil.renderGhosts(AutoAnchor.INSTANCE.espLength.getValueInt(), AutoAnchor.INSTANCE.espFactor.getValueInt(), AutoAnchor.INSTANCE.espShaking.getValueFloat(), AutoAnchor.INSTANCE.espAmplitude.getValueFloat(), AutoAnchor.INSTANCE.ghostsColor.getValue(), entity);
            case CaptureMark -> TargetESPUtil.renderCaptureMark(entity);
        }
    }
    ...
    private boolean check() {
        ...
        at = (int) (at * Catrix.SERVER.getTPSFactor());
