...
import me.catrix.api.utils.math.ExplosionUtil;
...
public class AutoAnchor extends Module {
	...
	private final EnumSetting<KillAura.TargetESP> mode = add(new EnumSetting<>("TargetESP", KillAura.TargetESP.CaptureMark, () -> page.getValue() == Page.Render));
	private final ColorSetting color = add(new ColorSetting("TargetColor", new Color(0x6CFFFFFF,true), () -> page.getValue() == Page.Render));

    public final ColorSetting ghostsColor = add(new ColorSetting("GhostsColor", new Color(174, 24, 234, 200), () -> page.getValue() == Page.Render && mode.is(KillAura.TargetESP.Ghosts)));
    public final SliderSetting espLength = add(new SliderSetting("ESPLength", 14, 1, 40, () -> page.getValue() == Page.Render && mode.is(KillAura.TargetESP.Ghosts)));
    public final SliderSetting espFactor = add(new SliderSetting("ESPFactor", 8, 1, 20, () -> page.getValue() == Page.Render && mode.is(KillAura.TargetESP.Ghosts)));
    public final SliderSetting espShaking = add(new SliderSetting("ESPShaking", 1.8f, 1.5f, 10f, () -> page.getValue() == Page.Render && mode.is(KillAura.TargetESP.Ghosts)));
    public final SliderSetting espAmplitude = add(new SliderSetting("ESPAmplitude", 3f, 0.1f, 8f, () -> page.getValue() == Page.Render && mode.is(KillAura.TargetESP.Ghosts)));
	...
	public AutoAnchor() {
		...
		setChinese("自动重生锚");
		INSTANCE = this;
		Catrix.EVENT_BUS.subscribe(new AnchorRender());
	}
	...
	public void onUpdate() {
		...
		if (currentPos != null) {
			...
			boolean shouldSpam = this.spam.getValue() && (!mineSpam.getValue() || Catrix.BREAK.isMining(currentPos, false));
			if (shouldSpam) {
	...
	public BlockPos getHelper(BlockPos pos) {
		for (Direction i : Direction.values()) {
			if (checkMine.getValue() && Catrix.BREAK.isMining(pos.offset(i))) continue;
