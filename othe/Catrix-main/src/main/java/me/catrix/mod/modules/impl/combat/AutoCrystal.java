...
import static me.catrix.api.utils.world.BlockUtil.hasCrystal;
...
public class AutoCrystal extends Module {
    ...
    private final SliderSetting priority = add(new SliderSetting("Priority", 20,0 ,100, () -> rotate.isOpen() && yawStep.getValue() && page.getValue() == Page.Rotation));
    //Place
    ...
    private final EnumSetting<TargetESP> mode = add(new EnumSetting<>("TargetESP", TargetESP.Ghosts, () -> page.getValue() == Page.Render));
    ...
    private final ColorSetting ghostsColor = add(new ColorSetting("GhostsColor", new Color(174, 24, 234, 200), () -> page.getValue() == Page.Render && mode.is(TargetESP.Ghosts)));
    public final SliderSetting espLength = add(new SliderSetting("ESPLength", 14, 1, 40, () -> page.getValue() == Page.Render && mode.is(TargetESP.Ghosts)));
    public final SliderSetting espFactor = add(new SliderSetting("ESPFactor", 8, 1, 20, () -> page.getValue() == Page.Render && mode.is(TargetESP.Ghosts)));
    public final SliderSetting espShaking = add(new SliderSetting("ESPShaking", 1.8f, 1.5f, 10f, () -> page.getValue() == Page.Render && mode.is(TargetESP.Ghosts)));
    public final SliderSetting espAmplitude = add(new SliderSetting("ESPAmplitude", 3f, 0.1f, 8f, () -> page.getValue() == Page.Render && mode.is(TargetESP.Ghosts)));
    //Calc
    ...
    public final BooleanSetting forceWeb = add(new BooleanSetting("ForceWeb", true, () -> page.getValue() == Page.Misc).setParent());
    ...
    public AutoCrystal() {
        ...
        setChinese("自动水晶");
        INSTANCE = this;
        Catrix.EVENT_BUS.subscribe(new CrystalRender());
    }
    ...
    public void doRender(MatrixStack matrixStack, float partialTicks, Entity entity, TargetESP mode) {
        switch (mode) {
            ...
            case Jello -> TargetESPUtil.drawJello(matrixStack, entity, color.getValue());
            case Ghosts -> TargetESPUtil.renderGhosts(espLength.getValueInt(), espFactor.getValueInt(), espShaking.getValueFloat(), espAmplitude.getValueFloat(), ghostsColor.getValue(), entity);
            case CaptureMark -> TargetESPUtil.renderCaptureMark(entity);
        }
    }
        ...
        if (list.isEmpty()) {
            ...
            for (Entity entity : mc.world.getEntities()) {
                if (entity instanceof EndCrystalEntity crystal && crystal.isAlive()) {
                    double distance = mc.player.getEyePos().distanceTo(crystal.getPos());
    ...
    private void doBreak(BlockPos pos) {
        ...
        for (EndCrystalEntity entity : BlockUtil.getEndCrystals(new Box(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 2, pos.getZ() + 1))) {
            ...
            if (rotate.getValue() && onBreak.getValue()) {
                if (!faceVector(entity.getPos().add(0, yOffset.getValue(), 0))) {
                    if (checkFov.getValue()) continue;
                }
            }
            ...
            if (crystalPos != null && displayTarget != null && lastDamage >= getDamage(displayTarget) * 0.8f && afterBreak.getValue()) {
                if (!yawStep.getValue() || !checkFov.getValue() || Catrix.ROTATION.inFov(entity.getPos(), fov.getValueFloat() + 10)) { // 放宽FOV限制
                    doPlace(crystalPos);
                }
            }
