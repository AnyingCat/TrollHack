...
public class CatMenu2 extends Screen implements Wrapper {
    public CatMenu2() {
        super(Text.translatable("narrator.screen.title"));
    }

    public static final Identifier icon = new  Identifier( "textures/icon.png");

    @Override
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        RenderShadersUtil.drawMainMenuShader2(context.getMatrices(), 0, 0, mc.getWindow().getScaledWidth(), mc.getWindow().getScaledHeight());
        context.drawTextWithShadow(this.textRenderer, "免费社区版，倒卖死全家，官方群:1072262390", 2, height - 20, new Color(0xFFFFFF).getRGB());
        context.drawTexture(icon, (width - 200) / 2, Math.min(height / 4, height / 2 - 80), 0, 0, 200, 100, 200, 100);
