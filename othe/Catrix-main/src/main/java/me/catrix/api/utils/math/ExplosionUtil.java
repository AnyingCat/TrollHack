...
import me.catrix.core.impl.PlayerManager;
...
public class ExplosionUtil implements Wrapper {
    ...
    public static float calculateDamage(double posX, double posY, double posZ, LivingEntity entity, Entity predict, float power) {
        ...
        double distancedsize = MathHelper.sqrt((float) predict.squaredDistanceTo(posX, posY, posZ)) / doubleExplosionSize;
        Vec3d vec3d = new Vec3d(posX, posY, posZ);
        double blockDensity = getExposure(vec3d, predict);
        double v = (1.0 - distancedsize) * blockDensity;
        float damage = (int) ((v * v + v) / 2.0 * 7.0 * doubleExplosionSize + 1.0);
    ...
    public static float getBlastReduction(LivingEntity entity, float damageI) {
        ...
        if (entity instanceof PlayerEntity player) {
            PlayerManager.EntityAttribute a = Catrix.PLAYER.map.get(player);
            if (a == null) return 0;
    ...
    public static float calculateCrystalDamage(Vec3d crystalPos, LivingEntity target, Entity predictor, float power) {
        if (target instanceof PlayerEntity player && player.getAbilities().creativeMode) return 0;
        if (predictor == null) predictor = target;

        double distance = Math.sqrt(predictor.squaredDistanceTo(crystalPos));
        double scaledDistance = distance / (2 * power);
//        double exposure = getExposure(crystalPos, predictor);

        double distanceFactor = 1.0 - scaledDistance;
        double damageFactor = (distanceFactor * distanceFactor + distanceFactor) / 2.0;
        float baseDamage = (float) (damageFactor * 7.0 * (2 * power) + 1.0);

        return getBlastReduction(target, getDamageMultiplied(baseDamage));
    }

    public static float calculateAnchorDamage(BlockPos anchorPos, PlayerEntity target, PlayerEntity predictor) {
        boolean isAnchor = BlockUtil.getBlock(anchorPos) == Blocks.RESPAWN_ANCHOR;

        if (isAnchor) {
            CombatUtil.modifyPos = anchorPos;
            CombatUtil.modifyBlockState = Blocks.AIR.getDefaultState();
        }

        Vec3d centerPos = anchorPos.toCenterPos();
        float damage = calculateCrystalDamage(centerPos, target, predictor, 5.0f);
