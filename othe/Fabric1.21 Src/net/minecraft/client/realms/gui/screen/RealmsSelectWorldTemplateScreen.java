...
extends RealmsScreen {
    ...
    private void renderMessages(DrawContext context, int x, int y, List<TextRenderingUtils.Line> messages) {
        for (int i = 0; i < messages.size(); ++i) {
            ...
            for (TextRenderingUtils.LineSegment lineSegment : line.segments) {
                int m = lineSegment.isLink() ? 0x3366BB : Colors.WHITE;
    ...
    extends AlwaysSelectedEntryListWidget.Entry<WorldTemplateObjectSelectionListEntry> {
        ...
        public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            context.drawText(RealmsSelectWorldTemplateScreen.this.textRenderer, this.mTemplate.version, x + entryWidth - j - 5, l, 0x6C6C6C, false);
            context.drawText(RealmsSelectWorldTemplateScreen.this.textRenderer, this.mTemplate.author, k, l + ((RealmsSelectWorldTemplateScreen)RealmsSelectWorldTemplateScreen.this).textRenderer.fontHeight + 5, Colors.LIGHT_GRAY, false);
            if (!this.mTemplate.recommendedPlayers.isBlank()) {
                context.drawText(RealmsSelectWorldTemplateScreen.this.textRenderer, this.mTemplate.recommendedPlayers, k, y + entryHeight - ((RealmsSelectWorldTemplateScreen)RealmsSelectWorldTemplateScreen.this).textRenderer.fontHeight / 2 - 5, 0x4C4C4C, false);
            }
        }
