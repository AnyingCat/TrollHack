...
extends RealmsScreen {
    ...
    public void init() {
        ...
        if (this.serverData.expired) {
            this.addDrawableChild(ButtonWidget.builder(Text.translatable("mco.configure.world.delete.button"), button -> this.client.setScreen(RealmsPopups.createContinuableWarningPopup(this, Text.translatable("mco.configure.world.delete.question.line1"), popupScreen -> this.onDeletionConfirmed()))).dimensions(this.width / 2 - 100, RealmsSubscriptionInfoScreen.row(10), 200, 20).build());
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawText(this.textRenderer, SUBSCRIPTION_START_LABEL_TEXT, i, RealmsSubscriptionInfoScreen.row(0), Colors.LIGHT_GRAY, false);
        context.drawText(this.textRenderer, this.startDate, i, RealmsSubscriptionInfoScreen.row(1), Colors.WHITE, false);
        if (this.type == Subscription.SubscriptionType.NORMAL) {
            context.drawText(this.textRenderer, TIME_LEFT_LABEL_TEXT, i, RealmsSubscriptionInfoScreen.row(3), Colors.LIGHT_GRAY, false);
        } else if (this.type == Subscription.SubscriptionType.RECURRING) {
            context.drawText(this.textRenderer, DAYS_LEFT_LABEL_TEXT, i, RealmsSubscriptionInfoScreen.row(3), Colors.LIGHT_GRAY, false);
        }
        context.drawText(this.textRenderer, this.daysLeft, i, RealmsSubscriptionInfoScreen.row(4), Colors.WHITE, false);
    }
    ...
    private Text daysLeftPresentation(int daysLeft) {
        ...
        if (bl2) {
            return Text.translatable("mco.configure.world.subscription.remaining.days", j);
        }
        return Text.empty();
    }
}
