...
extends HandledScreen<LoomScreenHandler> {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        if (this.bannerPatterns != null && !this.hasTooManyPatterns) {
            ...
            BannerBlockEntityRenderer.renderCanvas(context.getMatrices(), context.getVertexConsumers(), 0xF000F0, OverlayTexture.DEFAULT_UV, this.bannerField, ModelLoader.BANNER_BASE, true, dyeColor, this.bannerPatterns);
        ...
        if (this.canApplyDyePattern) {
            ...
            block0: for (int n = 0; n < 4; ++n) {
                for (int o = 0; o < 4; ++o) {
                    ...
                    Identifier identifier2 = q == ((LoomScreenHandler)this.handler).getSelectedPattern() ? PATTERN_SELECTED_TEXTURE : (bl ? PATTERN_HIGHLIGHTED_TEXTURE : PATTERN_TEXTURE);
    ...
    private void drawBanner(DrawContext context, RegistryEntry<BannerPattern> pattern, int x, int y) {
        ...
        BannerBlockEntityRenderer.renderCanvas(matrixStack, context.getVertexConsumers(), 0xF000F0, OverlayTexture.DEFAULT_UV, this.bannerField, ModelLoader.BANNER_BASE, true, DyeColor.GRAY, bannerPatternsComponent);
        matrixStack.pop();
        context.draw();
    }
