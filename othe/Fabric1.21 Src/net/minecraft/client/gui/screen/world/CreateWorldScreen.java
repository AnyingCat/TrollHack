...
extends Screen {
    ...
    private void applyDataPacks(ResourcePackManager dataPackManager, boolean fromPackScreen, Consumer<DataConfiguration> configurationSetter) {
        ...
        DataConfiguration dataConfiguration = new DataConfiguration(new DataPackSettings(list, list2 = (List)dataPackManager.getIds().stream().filter(name -> !list.contains(name)).collect(ImmutableList.toImmutableList())), this.worldCreator.getGeneratorOptionsHolder().dataConfiguration().enabledFeatures());
    ...
    private void validateDataPacks(ResourcePackManager dataPackManager, DataConfiguration dataConfiguration, Consumer<DataConfiguration> configurationSetter) {
        ...
        ((CompletableFuture)((CompletableFuture)SaveLoading.load(serverConfig, context -> {
            ...
            GeneratorOptionsHolder generatorOptionsHolder = this.worldCreator.getGeneratorOptionsHolder();
            RegistryOps<JsonElement> dynamicOps = generatorOptionsHolder.getCombinedRegistryManager().getOps(JsonOps.INSTANCE);
    ...
    private Optional<LevelStorage.Session> createSession() {
        ...
        block12: {
            ...
            block11: {
                ...
                session = this.client.getLevelStorage().createSessionWithoutSymlinkCheck(string);
