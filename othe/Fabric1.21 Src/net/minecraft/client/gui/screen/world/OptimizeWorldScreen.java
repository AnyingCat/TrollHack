...
extends Screen {
    ...
    public static OptimizeWorldScreen create(MinecraftClient client, BooleanConsumer callback, DataFixer dataFixer, LevelStorage.Session storageSession, boolean eraseCache) {
        ...
        try {
            ...
            OptimizeWorldScreen optimizeWorldScreen = new OptimizeWorldScreen(callback, dataFixer, storageSession, saveProperties.getLevelInfo(), eraseCache, immutable);
            if (saveLoader != null) {
                saveLoader.close();
            }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (this.updater.getTotalChunkCount() > 0) {
            ...
            context.drawCenteredTextWithShadow(this.textRenderer, text, this.width / 2, k + 2 * this.textRenderer.fontHeight + 2, 0xA0A0A0);
            context.drawCenteredTextWithShadow(this.textRenderer, text2, this.width / 2, k + (l - k) / 2 - this.textRenderer.fontHeight / 2, 0xA0A0A0);
        }
    }
}
