...
RecipeDisplayListener {
    ...
    private void refreshResults(boolean resetCurrentPage) {
        ...
        if (!string.isEmpty() && (clientPlayNetworkHandler = this.client.getNetworkHandler()) != null) {
            ObjectLinkedOpenHashSet<RecipeResultCollection> objectSet = new ObjectLinkedOpenHashSet<RecipeResultCollection>(clientPlayNetworkHandler.getSearchManager().getRecipeOutputReloadFuture().findAll(string.toLowerCase(Locale.ROOT)));
