...
public class InGameHud {
    ...
    public InGameHud(MinecraftClient client) {
        ...
        this.layeredDrawer.addSubDrawer(layeredDrawer, () -> !minecraftClient.options.hudHidden).addLayer(this::renderSleepOverlay).addSubDrawer(layeredDrawer2, () -> !minecraftClient.options.hudHidden);
    }
    ...
    private void renderScoreboardSidebar(DrawContext context, ScoreboardObjective objective) {
        ...
        context.draw(() -> {
            ...
            context.drawText(this.getTextRenderer(), text, o + l / 2 - i / 2, s - this.getTextRenderer().fontHeight, Colors.WHITE, false);
            for (int t = 0; t < k; ++t) {
                SidebarEntry sidebarEntry = sidebarEntrys[t];
                int u = m - (k - t) * this.getTextRenderer().fontHeight;
                context.drawText(this.getTextRenderer(), sidebarEntry.name, o, u, Colors.WHITE, false);
                context.drawText(this.getTextRenderer(), sidebarEntry.score, p - sidebarEntry.scoreWidth, u, Colors.WHITE, false);
            }
        });
    }
    ...
    private void renderVignetteOverlay(DrawContext context, @Nullable Entity entity) {
        ...
        float f = 0.0f;
        if (entity != null) {
            g = (float)worldBorder.getDistanceInsideBorder(entity);
