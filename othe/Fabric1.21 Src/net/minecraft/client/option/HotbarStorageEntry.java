...
public class HotbarStorageEntry {
    ...
    public void serialize(PlayerInventory playerInventory, DynamicRegistryManager registryManager) {
        ...
        for (int i = 0; i < HOTBAR_SIZE; ++i) {
            ...
            Optional<Dynamic> optional = ItemStack.OPTIONAL_CODEC.encodeStart(registryOps, itemStack).resultOrPartial(error -> LOGGER.warn("Could not encode hotbar item: {}", error)).map(nbt -> new Dynamic<NbtElement>(NBT_OPS, (NbtElement)nbt));
