...
implements BlockEntityRenderer<StructureBlockBlockEntity> {
    ...
    private void method_61047(StructureBlockBlockEntity entity, VertexConsumer vertexConsumer, MatrixStack matrices) {
        ...
        BitSetVoxelSet voxelSet = new BitSetVoxelSet(vec3i.getX(), vec3i.getY(), vec3i.getZ());
        for (BlockPos blockPos3 : BlockPos.iterate(blockPos2, blockPos2.add(vec3i).add(-1, -1, -1))) {
            if (!blockView.getBlockState(blockPos3).isOf(Blocks.STRUCTURE_VOID)) continue;
            ((VoxelSet)voxelSet).set(blockPos3.getX() - blockPos2.getX(), blockPos3.getY() - blockPos2.getY(), blockPos3.getZ() - blockPos2.getZ());
        }
