...
extends MobEntityRenderer<T, SquidEntityModel<T>> {
    ...
    protected void setupTransforms(T squidEntity, MatrixStack matrixStack, float f, float g, float h, float i) {
        float j = MathHelper.lerp(h, ((SquidEntity)squidEntity).prevTiltAngle, ((SquidEntity)squidEntity).tiltAngle);
        float k = MathHelper.lerp(h, ((SquidEntity)squidEntity).prevRollAngle, ((SquidEntity)squidEntity).rollAngle);
