...
extends EntityRenderer<T> {
    ...
    public void render(T itemFrameEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        ...
        if (!itemStack.isEmpty()) {
            MapIdComponent mapIdComponent = ((ItemFrameEntity)itemFrameEntity).getMapId(itemStack);
            if (bl) {
                matrixStack.translate(0.0f, 0.0f, 0.5f);
            } else {
                matrixStack.translate(0.0f, 0.0f, 0.4375f);
            }
            int j = mapIdComponent != null ? ((ItemFrameEntity)itemFrameEntity).getRotation() % 4 * 2 : ((ItemFrameEntity)itemFrameEntity).getRotation();
            ...
            if (mapIdComponent != null) {
                ...
                MapState mapState = FilledMapItem.getMapState(mapIdComponent, ((Entity)itemFrameEntity).getWorld());
    ...
    protected boolean hasLabel(T itemFrameEntity) {
        if (!MinecraftClient.isHudEnabled() || ((ItemFrameEntity)itemFrameEntity).getHeldItemStack().isEmpty() || !((ItemFrameEntity)itemFrameEntity).getHeldItemStack().contains(DataComponentTypes.CUSTOM_NAME) || this.dispatcher.targetedEntity != itemFrameEntity) {
            return false;
        }
    ...
    protected void renderLabelIfPresent(T itemFrameEntity, Text text, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, float f) {
        super.renderLabelIfPresent(itemFrameEntity, ((ItemFrameEntity)itemFrameEntity).getHeldItemStack().getName(), matrixStack, vertexConsumerProvider, i, f);
    }
}
