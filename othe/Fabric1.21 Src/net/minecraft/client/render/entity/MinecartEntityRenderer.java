...
extends EntityRenderer<T> {
    ...
    public void render(T abstractMinecartEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        ...
        float p = (float)((VehicleEntity)abstractMinecartEntity).getDamageWobbleTicks() - g;
        float q = ((VehicleEntity)abstractMinecartEntity).getDamageWobbleStrength() - g;
        if (q < 0.0f) {
            q = 0.0f;
        }
        if (p > 0.0f) {
            matrixStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(MathHelper.sin(p) * p * q / 10.0f * (float)((VehicleEntity)abstractMinecartEntity).getDamageWobbleSide()));
        }
