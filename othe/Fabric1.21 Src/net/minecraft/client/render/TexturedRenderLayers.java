...
public class TexturedRenderLayers {
    ...
    public static final SpriteIdentifier[] BED_TEXTURES = (SpriteIdentifier[])Arrays.stream(DyeColor.values()).sorted(Comparator.comparingInt(DyeColor::getId)).map(color -> new SpriteIdentifier(BEDS_ATLAS_TEXTURE, Identifier.ofVanilla("entity/bed/" + color.getName()))).toArray(SpriteIdentifier[]::new);
