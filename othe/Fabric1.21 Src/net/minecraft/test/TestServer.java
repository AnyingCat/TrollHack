...
extends MinecraftServer {
    ...
    public static TestServer create(Thread thread, LevelStorage.Session session, ResourcePackManager resourcePackManager, Collection<TestFunction> batches, BlockPos pos) {
        ...
        DataConfiguration dataConfiguration = new DataConfiguration(new DataPackSettings(new ArrayList<String>(resourcePackManager.getIds()), List.of()), FeatureFlags.FEATURE_MANAGER.getFeatureSet());
        ...
        try {
            ...
            SaveLoader saveLoader = (SaveLoader)Util.waitAndApply(executor -> SaveLoading.load(serverConfig, context -> {
                ...
                DimensionOptionsRegistryHolder.DimensionsConfig dimensionsConfig = context.worldGenRegistryManager().get(RegistryKeys.WORLD_PRESET).entryOf(WorldPresets.FLAT).value().createDimensionsRegistryHolder().toConfig(registry);
