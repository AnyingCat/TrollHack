...
public class TestContext {
    ...
    public void useBlock(BlockPos pos, PlayerEntity player, BlockHitResult result) {
        ...
        BlockPos blockPos = this.getAbsolutePos(pos);
        BlockState blockState = this.getWorld().getBlockState(blockPos);
        ItemActionResult itemActionResult = blockState.onUseWithItem(player.getStackInHand(hand = Hand.MAIN_HAND), this.getWorld(), player, hand, result);
    ...
    public <T extends Comparable<T>> void expectBlockProperty(BlockPos pos, Property<T> property, T value) {
        ...
        if (!bl || !blockState.get(property).equals(value)) {
            String string = bl ? "was " + String.valueOf(blockState.get(property)) : "property " + property.getName() + " is missing";
    ...
    public void expectEntityToTouch(EntityType<?> type, double x, double y, double z) {
        ...
        if (list.isEmpty()) {
            throw new GameTestException("Expected " + type.getUntranslatedName() + " to touch " + String.valueOf(vec3d2) + " (relative " + String.valueOf(vec3d) + ")");
        }
    }
    ...
    public void dontExpectEntityToTouch(EntityType<?> type, double x, double y, double z) {
        ...
        List<Entity> list = this.getWorld().getEntitiesByType(type, this.getTestBox(), predicate);
        if (list.isEmpty()) {
            throw new GameTestException("Did not expect " + type.getUntranslatedName() + " to touch " + String.valueOf(vec3d2) + " (relative " + String.valueOf(vec3d) + ")");
        }
    }
    ...
    public <E extends Entity, T> void expectEntityWithData(BlockPos pos, EntityType<E> type, Function<? super E, T> entityDataGetter, @Nullable T data) {
        ...
        for (Entity entity : list) {
            ...
            throw new GameTestException("Expected entity data to be: " + String.valueOf(data) + ", but was: " + String.valueOf(object));
        }
    }
    ...
    public <E extends LivingEntity> void expectEntityHoldingItem(BlockPos pos, EntityType<E> entityType, Item item) {
        ...
        if (list.isEmpty()) {
            throw new PositionedException("Expected entity of type: " + String.valueOf(entityType), blockPos, pos, this.getTick());
        }
        for (LivingEntity livingEntity : list) {
            if (!livingEntity.isHolding(item)) continue;
            return;
        }
        throw new PositionedException("Entity should be holding: " + String.valueOf(item), blockPos, pos, this.getTick());
    }
    ...
    public <E extends Entity> void expectEntityWithItem(BlockPos pos, EntityType<E> entityType, Item item) {
        ...
        throw new PositionedException("Entity inventory should contain: " + String.valueOf(item), blockPos, pos, this.getTick());
    }
    ...
    public void expectContainerWith(BlockPos pos, Item item) {
        ...
        if (!(blockEntity instanceof LockableContainerBlockEntity)) {
            throw new GameTestException("Expected a container at " + String.valueOf(pos) + ", found " + String.valueOf(Registries.BLOCK_ENTITY_TYPE.getId(blockEntity.getType())));
        }
    ...
    public void expectSameStates(BlockPos checkedPos, BlockPos correctStatePos) {
        ...
        if (blockState != (blockState2 = this.getBlockState(correctStatePos))) {
            this.throwPositionedException("Incorrect state. Expected " + String.valueOf(blockState2) + ", got " + String.valueOf(blockState), checkedPos);
        }
    }
    ...
    public <E extends Entity, T> void testEntityProperty(E entity, Function<E, T> propertyGetter, String propertyName, T expectedValue) {
        ...
        if (!object.equals(expectedValue)) {
            throw new GameTestException("Entity " + String.valueOf(entity) + " value " + propertyName + "=" + String.valueOf(object) + " is not equal to expected " + String.valueOf(expectedValue));
        }
    }
    ...
    public void expectEntityHasEffect(LivingEntity entity, RegistryEntry<StatusEffect> effect, int amplifier) {
        ...
        if (statusEffectInstance == null || statusEffectInstance.getAmplifier() != amplifier) {
            int i = amplifier + 1;
            throw new GameTestException("Entity " + String.valueOf(entity) + " failed has " + effect.value().getTranslationKey() + " x " + i + " test");
        }
    }
    ...
    public void setBiome(RegistryKey<Biome> biome) {
        ...
        Either<Integer, CommandSyntaxException> either = FillBiomeCommand.fillBiome(this.getWorld(), blockPos, blockPos2, this.getWorld().getRegistryManager().get(RegistryKeys.BIOME).entryOf(biome));
