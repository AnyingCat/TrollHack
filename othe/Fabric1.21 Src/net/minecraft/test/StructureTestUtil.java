...
public class StructureTestUtil {
    ...
    public static BlockBox getStructureBlockBox(StructureBlockBlockEntity structureBlockEntity) {
        BlockPos blockPos = StructureTestUtil.getStructureBlockPos(structureBlockEntity);
        BlockPos blockPos2 = StructureTestUtil.getStructureBlockBoxCornerPos(blockPos, structureBlockEntity.getSize(), structureBlockEntity.getRotation());
    ...
    public static void createTestArea(String testName, BlockPos pos, Vec3i relativePos, BlockRotation rotation, ServerWorld world) {
        BlockBox blockBox = StructureTestUtil.getStructureBlockBox(pos.up(), relativePos, rotation);
    ...
    public static StructureBlockBlockEntity initStructure(GameTestState state, BlockPos pos, BlockRotation rotation, ServerWorld world) {
        BlockPos blockPos;
        Vec3i vec3i = world.getStructureTemplateManager().getTemplate(Identifier.of(state.getTemplateName())).orElseThrow(() -> new IllegalStateException("Missing test structure: " + state.getTemplateName())).getSize();
        ...
        StructureTestUtil.forceLoadNearbyChunks(blockBox, world);
        StructureTestUtil.clearArea(blockBox, world);
        return StructureTestUtil.placeStructureTemplate(state, blockPos.down(), rotation, world);
    }
    ...
    public static void clearBarrierBox(Box box, ServerWorld world) {
        ...
        BlockPos.stream(blockPos, blockPos2).forEach(pos -> {
            ...
            boolean bl3 = bl2 = pos.getY() == blockPos2.getY();
    ...
    public static void clearArea(BlockBox area, ServerWorld world) {
        ...
        BlockPos.stream(blockBox).forEach(pos -> StructureTestUtil.resetBlock(i, pos, world));
    ...
    public static BlockBox getStructureBlockBox(BlockPos pos, Vec3i relativePos, BlockRotation rotation) {
        BlockPos blockPos = StructureTestUtil.getStructureBlockBoxCornerPos(pos, relativePos, rotation);
    ...
    public static Stream<BlockPos> findStructureBlocks(BlockPos pos, int radius, ServerWorld world) {
        BlockBox blockBox = StructureTestUtil.createBox(pos, radius, world);
        return BlockPos.stream(blockBox).filter(p -> world.getBlockState((BlockPos)p).isOf(Blocks.STRUCTURE_BLOCK)).map(BlockPos::toImmutable);
    }
    ...
    private static boolean isInStructureBounds(BlockPos structureBlockPos, BlockPos pos, ServerWorld world) {
        ...
        return StructureTestUtil.getStructureBlockBox(structureBlockBlockEntity).contains(pos);
    }
}
