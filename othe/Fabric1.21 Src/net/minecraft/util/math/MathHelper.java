...
public class MathHelper {
    ...
    public static int hsvToArgb(float hue, float saturation, float value, int alpha) {
        ...
        return ColorHelper.Argb.getArgb(alpha, MathHelper.clamp((int)(k * 255.0f), 0, 255), MathHelper.clamp((int)(l * 255.0f), 0, 255), MathHelper.clamp((int)((switch (i) {
            case 0 -> {
                k = value;
                l = j;
                yield g;
    ...
    public static IntStream stream(int seed, int lowerBound, int upperBound, int steps) {
        ...
        return IntStream.iterate(seed, i -> {
            int m = java.lang.Math.abs(seed - i);
            return seed - m >= lowerBound || seed + m <= upperBound;
        }, i -> {
            int o;
            boolean bl2;
            boolean bl = i <= seed;
            int n = java.lang.Math.abs(seed - i);
