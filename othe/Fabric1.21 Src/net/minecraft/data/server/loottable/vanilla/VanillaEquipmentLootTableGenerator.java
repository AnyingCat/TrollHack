...
public record VanillaEquipmentLootTableGenerator(RegistryWrapper.WrapperLookup registries) implements LootTableGenerator
{
    ...
    public void accept(BiConsumer<RegistryKey<LootTable>, LootTable.Builder> lootTableBiConsumer) {
        RegistryWrapper.Impl<ArmorTrimPattern> impl = this.registries.getOptionalWrapper(RegistryKeys.TRIM_PATTERN).orElseThrow();
        RegistryWrapper.Impl<ArmorTrimMaterial> impl2 = this.registries.getOptionalWrapper(RegistryKeys.TRIM_MATERIAL).orElseThrow();
        ...
        lootTableBiConsumer.accept(LootTables.TRIAL_CHAMBER_EQUIPMENT, LootTable.builder().pool(LootPool.builder().rolls(ConstantLootNumberProvider.create(1.0f)).with((LootPoolEntry.Builder<?>)LootTableEntry.builder(VanillaEquipmentLootTableGenerator.createEquipmentTableBuilder(Items.CHAINMAIL_HELMET, Items.CHAINMAIL_CHESTPLATE, armorTrim2, impl3).build()).weight(4)).with((LootPoolEntry.Builder<?>)LootTableEntry.builder(VanillaEquipmentLootTableGenerator.createEquipmentTableBuilder(Items.IRON_HELMET, Items.IRON_CHESTPLATE, armorTrim, impl3).build()).weight(2)).with((LootPoolEntry.Builder<?>)LootTableEntry.builder(VanillaEquipmentLootTableGenerator.createEquipmentTableBuilder(Items.DIAMOND_HELMET, Items.DIAMOND_CHESTPLATE, armorTrim, impl3).build()).weight(1))));
