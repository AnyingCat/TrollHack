    ...
    public void accept(BiConsumer<RegistryKey<LootTable>, LootTable.Builder> lootTableBiConsumer) {
        ...
        Registries.ENTITY_TYPE.streamEntries().forEach(entityType -> {
            ...
            if (EntityLootTableGenerator.shouldCheck(entityType2)) {
                ...
                RegistryKey<LootTable> registryKey2 = entityType2.getLootTableId();
                if (registryKey2 != LootTables.EMPTY && entityType2.isEnabled(this.featureSet) && (map == null || !map.containsKey(registryKey2))) {
                    throw new IllegalStateException(String.format(Locale.ROOT, "Missing loottable '%s' for '%s'", registryKey2, entityType.registryKey().getValue()));
                }
                if (map != null) {
                ...
                Map<RegistryKey<LootTable>, LootTable.Builder> map = this.lootTables.remove(entityType2);
                if (map != null) {
                    throw new IllegalStateException(String.format(Locale.ROOT, "Weird loottables '%s' for '%s', not a LivingEntity so should not have loot", map.keySet().stream().map(registryKey -> registryKey.getValue().toString()).collect(Collectors.joining(",")), entityType.registryKey().getValue()));
                }
            }
        });
