    ...
    public CompletableFuture<?> run(DataWriter writer) {
        ...
        return this.registryLookupFuture.thenCompose(registryLookup -> {
            ...
            registryLookup.getWrapperOrThrow(RegistryKeys.ITEM).streamEntries().forEach(entry -> {
                ...
                jsonObject2.add("components", (JsonElement)ComponentMap.CODEC.encodeStart(registryOps, ((Item)entry.value()).getComponents()).getOrThrow(components -> new IllegalStateException("Failed to encode components: " + components)));
