...
public class Main {
    ...
    public static DataGenerator create(Path output, Collection<Path> inputs, boolean includeClient, boolean includeServer, boolean includeDev, boolean includeReports, boolean validate, GameVersion gameVersion, boolean ignoreCache) {
        ...
        pack4.addProvider(Main.toFactory(DynamicRegistriesProvider::new, (CompletableFuture<RegistryWrapper.WrapperLookup>)completableFuture3));
        pack4.addProvider(outputx -> MetadataProvider.create(outputx, Text.translatable("dataPack.trade_rebalance.description"), FeatureSet.of(FeatureFlags.TRADE_REBALANCE)));
        pack4.addProvider(Main.toFactory(TradeRebalanceLootTableProviders::createTradeRebalanceProvider, completableFuture));
        pack4.addProvider(Main.toFactory(TradeRebalanceStructureTagProvider::new, completableFuture));
        pack4.addProvider(Main.toFactory(TradeRebalanceEnchantmentTagProvider::new, completableFuture));
