...
public class ChunkSerializer {
    ...
    public static ProtoChunk deserialize(ServerWorld world, PointOfInterestStorage poiStorage, StorageKey key, ChunkPos chunkPos, NbtCompound nbt) {
        ...
        for (int j = 0; j < nbtList.size(); ++j) {
            ...
            if (l >= 0 && l < chunkSections.length) {
                ...
                PalettedContainer palettedContainer = nbtCompound.contains("block_states", NbtElement.COMPOUND_TYPE) ? (PalettedContainer)CODEC.parse(NbtOps.INSTANCE, nbtCompound.getCompound("block_states")).promotePartial(errorMessage -> ChunkSerializer.logRecoverableError(chunkPos, k, errorMessage)).getOrThrow(ChunkLoadingException::new) : new PalettedContainer(Block.STATE_IDS, Blocks.AIR.getDefaultState(), PalettedContainer.PaletteProvider.BLOCK_STATE);
                ReadableContainer<RegistryEntry.Reference<Biome>> readableContainer = nbtCompound.contains("biomes", NbtElement.COMPOUND_TYPE) ? (ReadableContainer)codec.parse(NbtOps.INSTANCE, nbtCompound.getCompound("biomes")).promotePartial(errorMessage -> ChunkSerializer.logRecoverableError(chunkPos, k, errorMessage)).getOrThrow(ChunkLoadingException::new) : new PalettedContainer<RegistryEntry.Reference<Biome>>(registry.getIndexedEntries(), registry.entryOf(BiomeKeys.PLAINS), PalettedContainer.PaletteProvider.BIOME);
        ...
        ChunkType chunkType = ChunkSerializer.getChunkType(nbt);
        ...
        nbtList3 = nbt.getList("entities", NbtElement.COMPOUND_TYPE);
        for (o = 0; o < nbtList3.size(); ++o) {
    ...
    public static NbtCompound serialize(ServerWorld world, Chunk chunk) {
        ...
        for (BlockPos blockPos : chunk.getBlockEntityPositions()) {
            nbtCompound3 = chunk.getPackedBlockEntityNbt(blockPos, world.getRegistryManager());
