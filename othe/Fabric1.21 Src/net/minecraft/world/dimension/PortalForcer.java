...
public class PortalForcer {
    ...
    public Optional<BlockPos> getPortalPos(BlockPos pos, boolean destIsNether, WorldBorder worldBorder) {
        ...
        return pointOfInterestStorage.getInSquare(poiType -> poiType.matchesKey(PointOfInterestTypes.NETHER_PORTAL), pos, i, PointOfInterestStorage.OccupationStatus.ANY).map(PointOfInterest::getPos).filter(worldBorder::contains).filter(blockPos -> this.world.getBlockState((BlockPos)blockPos).contains(Properties.HORIZONTAL_AXIS)).min(Comparator.comparingDouble(blockPos2 -> blockPos2.getSquaredDistance(pos)).thenComparingInt(Vec3i::getY));
    }
