...
public class MultiNoiseBiomeSourceParameterList {
    ...
    public record Preset(Identifier id, BiomeSourceFunction biomeSourceFunction) {
        ...
        public static final Codec<Preset> CODEC = Identifier.CODEC.flatXmap(identifier -> Optional.ofNullable(BY_IDENTIFIER.get(identifier)).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Unknown preset: " + String.valueOf(identifier))), preset -> DataResult.success(preset.id));
