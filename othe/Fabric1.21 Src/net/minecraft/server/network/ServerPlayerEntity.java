...
extends PlayerEntity {
    ...
    public BlockPos getWorldSpawnPos(ServerWorld world, BlockPos basePos) {
        ...
        if (world.getDimension().hasSkyLight() && world.getServer().getSaveProperties().getGameMode() != GameMode.ADVENTURE) {
            ...
            for (int p = 0; p < k; ++p) {
                ...
                if (blockPos == null || !world.isSpaceEmpty(this, box.offset(blockPos.toBottomCenterPos()))) continue;
                return blockPos;
            }
            blockPos = basePos;
        }
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbtElement != null) {
            BlockPos.CODEC.parse(NbtOps.INSTANCE, nbtElement).resultOrPartial(LOGGER::error).ifPresent(startRaidPos -> {
    ...
    private void updateCreativeInteractionRangeModifiers() {
        ...
        if ((entityAttributeInstance2 = this.getAttributeInstance(EntityAttributes.PLAYER_ENTITY_INTERACTION_RANGE)) != null) {
            if (this.isCreative()) {
    ...
    public TeleportTarget getRespawnTarget(boolean alive, TeleportTarget.PostDimensionTransition postDimensionTransition) {
        ...
        if (serverWorld != null && blockPos != null) {
            Optional<RespawnPos> optional = ServerPlayerEntity.findRespawnPosition(serverWorld, blockPos, f, bl, alive);
    ...
    private static Optional<RespawnPos> findRespawnPosition(ServerWorld world, BlockPos pos, float spawnAngle, boolean spawnForced, boolean alive) {
        ...
        if (block instanceof BedBlock && BedBlock.isBedWorking(world)) {
            return BedBlock.findWakeUpPosition(EntityType.PLAYER, (CollisionView)world, pos, blockState.get(BedBlock.FACING), spawnAngle).map(respawnPos -> RespawnPos.fromCurrentPos(respawnPos, pos));
        }
        if (!spawnForced) {
    ...
    public void unlockRecipes(List<Identifier> recipes) {
        List<RecipeEntry<?>> list = recipes.stream().flatMap(recipe -> this.server.getRecipeManager().get((Identifier)recipe).stream()).collect(Collectors.toList());
    ...
    public SyncedClientOptions getClientOptions() {
        ...
        return new SyncedClientOptions(this.language, this.viewDistance, this.clientChatVisibility, this.clientChatColorsEnabled, i, arm, this.filterText, this.allowServerListing);
    }
