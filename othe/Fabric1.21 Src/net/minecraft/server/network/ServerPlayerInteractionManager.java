...
public class ServerPlayerInteractionManager {
    ...
    public ActionResult interactItem(ServerPlayerEntity player, World world, ItemStack stack, Hand hand) {
        ...
        if (typedActionResult.getResult() == ActionResult.FAIL && itemStack.getMaxUseTime(player) > 0 && !player.isUsingItem()) {
            return typedActionResult.getResult();
        }
        if (stack != itemStack) {
    ...
    public ActionResult interactBlock(ServerPlayerEntity player, World world, ItemStack stack, Hand hand, BlockHitResult hitResult) {
        ...
        if (!bl2) {
            ...
            if (itemActionResult == ItemActionResult.PASS_TO_DEFAULT_BLOCK_INTERACTION && hand == Hand.MAIN_HAND && (actionResult = blockState.onUse(world, player, hitResult)).isAccepted()) {
