...
public abstract class ChunkTicketManager {
    ...
    public <T> void addTicket(ChunkTicketType<T> type, ChunkPos pos, int radius, T argument) {
        ChunkTicket<T> chunkTicket = new ChunkTicket<T>(type, ChunkLevels.getLevelFromType(ChunkLevelType.FULL) - radius, argument);
        long l = pos.toLong();
    ...
    protected void setChunkForced(ChunkPos pos, boolean forced) {
        ChunkTicket<ChunkPos> chunkTicket = new ChunkTicket<ChunkPos>(ChunkTicketType.FORCED, ServerChunkLoadingManager.field_29670, pos);
        long l = pos.toLong();
        if (forced) {
            this.addTicket(l, chunkTicket);
            this.simulationDistanceTracker.add(l, chunkTicket);
