...
public abstract class PlayerManager {
    ...
    public void onPlayerConnect(ClientConnection connection, ServerPlayerEntity player, ConnectedClientData clientData) {
        ...
        if (userCache != null) {
            optional = userCache.getByUuid(gameProfile.getId());
        ...
        RegistryKey<World> registryKey = optional.flatMap(nbt -> DimensionType.worldFromDimensionNbt(new Dynamic<NbtElement>(NbtOps.INSTANCE, nbt.get("Dimension"))).resultOrPartial(LOGGER::error)).orElse(World.OVERWORLD);
        ...
        if (optional.isPresent() && ((NbtCompound)optional.get()).contains("RootVehicle", NbtElement.COMPOUND_TYPE) && (entity = EntityType.loadEntityWithPassengers((nbtCompound = ((NbtCompound)optional.get()).getCompound("RootVehicle")).getCompound("Entity"), serverWorld2, vehicle -> {
            if (!serverWorld2.tryLoadEntity((Entity)vehicle)) {
                return null;
            }
            return vehicle;
        })) != null) {
            UUID uUID = nbtCompound.containsUuid("Attach") ? nbtCompound.getUuid("Attach") : null;
    ...
    public ServerPlayerEntity respawnPlayer(ServerPlayerEntity player, boolean alive, Entity.RemovalReason removalReason) {
        ...
        if (!alive && (blockState = serverWorld.getBlockState(blockPos = BlockPos.ofFloored(teleportTarget.pos()))).isOf(Blocks.RESPAWN_ANCHOR)) {
