...
AttachmentTarget {
    ...
    public final NbtCompound createNbt(RegistryWrapper.WrapperLookup registryLookup) {
        ...
        Components.CODEC.encodeStart(registryLookup.getOps(NbtOps.INSTANCE), this.components).resultOrPartial(snbt -> LOGGER.warn("Failed to save components: {}", snbt)).ifPresent(nbt -> nbtCompound.copyFrom((NbtCompound)nbt));
        return nbtCompound;
    }
    ...
    public final void readComponents(ComponentMap defaultComponents, ComponentChanges components) {
        ...
        final ComponentMapImpl componentMap = ComponentMapImpl.create(defaultComponents, components);
