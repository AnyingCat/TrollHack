...
SingleStackInventory.SingleStackBlockEntityInventory {
    ...
    protected void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup) {
        ...
        this.recordStack = nbt.contains(RECORD_ITEM_NBT_KEY, NbtElement.COMPOUND_TYPE) ? ItemStack.fromNbt(registryLookup, nbt.getCompound(RECORD_ITEM_NBT_KEY)).orElse(ItemStack.EMPTY) : ItemStack.EMPTY;
        if (nbt.contains(TICKS_SINCE_SONG_STARTED_NBT_KEY, NbtElement.LONG_TYPE)) {
            JukeboxSong.getSongEntryFromStack(registryLookup, this.recordStack).ifPresent(song -> this.manager.setValues((RegistryEntry<JukeboxSong>)song, nbt.getLong(TICKS_SINCE_SONG_STARTED_NBT_KEY)));
        }
    }
    ...
    protected void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup) {
        ...
        if (!this.getStack().isEmpty()) {
            nbt.put(RECORD_ITEM_NBT_KEY, this.getStack().encode(registryLookup));
        }
