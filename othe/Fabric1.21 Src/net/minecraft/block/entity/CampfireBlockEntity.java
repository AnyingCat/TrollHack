...
implements Clearable {
    ...
    public static void litServerTick(World world, BlockPos pos, BlockState state, CampfireBlockEntity campfire) {
        ...
        for (int i = 0; i < campfire.itemsBeingCooked.size(); ++i) {
            ...
            if (campfire.cookingTimes[i] < campfire.cookingTotalTimes[i] || !(itemStack2 = campfire.matchGetter.getFirstMatch(singleStackRecipeInput = new SingleStackRecipeInput(itemStack), world).map(recipe -> ((CampfireCookingRecipe)recipe.value()).craft(singleStackRecipeInput, (RegistryWrapper.WrapperLookup)world.getRegistryManager())).orElse(itemStack)).isItemEnabled(world.getEnabledFeatures())) continue;
