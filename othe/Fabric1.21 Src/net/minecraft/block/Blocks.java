...
public class Blocks {
    ...
    public static final Block TRIAL_SPAWNER = Blocks.register("trial_spawner", (Block)new TrialSpawnerBlock(AbstractBlock.Settings.create().mapColor(MapColor.STONE_GRAY).instrument(NoteBlockInstrument.BASEDRUM).luminance(state -> state.get(TrialSpawnerBlock.TRIAL_SPAWNER_STATE).getLuminance()).strength(50.0f).sounds(BlockSoundGroup.TRIAL_SPAWNER).blockVision(Blocks::never).nonOpaque()));
    public static final Block VAULT = Blocks.register("vault", (Block)new VaultBlock(AbstractBlock.Settings.create().mapColor(MapColor.STONE_GRAY).instrument(NoteBlockInstrument.BASEDRUM).nonOpaque().sounds(BlockSoundGroup.VAULT).luminance(state -> state.get(VaultBlock.VAULT_STATE).getLuminance()).strength(50.0f).blockVision(Blocks::never)));
    ...
    private static Block createPistonBlock(boolean sticky) {
        ...
        return new PistonBlock(sticky, AbstractBlock.Settings.create().mapColor(MapColor.STONE_GRAY).strength(1.5f).solidBlock(Blocks::never).suffocates(contextPredicate).blockVision(contextPredicate).pistonBehavior(PistonBehavior.BLOCK));
    }
