...
public final class TrialSpawnerLogic {
    ...
    public Codec<TrialSpawnerLogic> codec() {
        return RecordCodecBuilder.create(instance -> instance.group(TrialSpawnerConfig.CODEC.optionalFieldOf(NORMAL_CONFIG_NBT_KEY, TrialSpawnerConfig.DEFAULT).forGetter(TrialSpawnerLogic::getNormalConfig), TrialSpawnerConfig.CODEC.optionalFieldOf(OMINOUS_CONFIG_NBT_KEY, TrialSpawnerConfig.DEFAULT).forGetter(TrialSpawnerLogic::getOminousConfigForSerialization), TrialSpawnerData.codec.forGetter(TrialSpawnerLogic::getData), Codec.intRange(0, Integer.MAX_VALUE).optionalFieldOf("target_cooldown_length", 36000).forGetter(TrialSpawnerLogic::getCooldownLength), Codec.intRange(1, 128).optionalFieldOf("required_player_range", 14).forGetter(TrialSpawnerLogic::getDetectionRadius)).apply((Applicative<TrialSpawnerLogic, ?>)instance, (config, trialSpawnerConfig, trialSpawnerData, integer, integer2) -> new TrialSpawnerLogic((TrialSpawnerConfig)config, (TrialSpawnerConfig)trialSpawnerConfig, (TrialSpawnerData)trialSpawnerData, (int)integer, (int)integer2, this.trialSpawner, this.entityDetector, this.entitySelector)));
    }
    ...
    public Optional<UUID> trySpawnMob(ServerWorld world, BlockPos pos) {
        ...
        double d2 = f = i >= 3 ? nbtList.getDouble(2) : (double)pos.getZ() + (random.nextDouble() - random.nextDouble()) * (double)this.getConfig().spawnRange() + 0.5;
        ...
        if (entity2 instanceof MobEntity) {
            ...
            boolean bl2 = bl = mobSpawnerEntry.getNbt().getSize() == 1 && mobSpawnerEntry.getNbt().contains("id", NbtElement.STRING_TYPE);
            if (bl) {
    ...
    public void ejectLootTable(ServerWorld world, BlockPos pos, RegistryKey<LootTable> lootTable) {
        ...
        ObjectArrayList<ItemStack> objectArrayList = lootTable2.generateLoot(lootContextParameterSet = new LootContextParameterSet.Builder(world).build(LootContextTypes.EMPTY));
