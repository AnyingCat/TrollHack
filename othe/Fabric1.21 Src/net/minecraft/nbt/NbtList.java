...
extends AbstractNbtList<NbtElement> {
    ...
    public static final NbtType<NbtList> TYPE = new NbtType.OfVariableSize<NbtList>(){
        ...
        private static NbtList readList(DataInput input, NbtSizeTracker tracker) throws IOException {
            ...
            for (int j = 0; j < i; ++j) {
                list.add((NbtElement)nbtType.read(input, tracker));
            }
        ...
        private static NbtScanner.Result scanList(DataInput input, NbtScanner visitor, NbtSizeTracker tracker) throws IOException {
             ...
             *     at net.fabricmc.loom.task.GenerateSourcesTask$DecompileAction.doDecompile(GenerateSourcesTask.java:642)
             *     at net.fabricmc.loom.task.GenerateSourcesTask$DecompileAction.execute(GenerateSourcesTask.java:609)
    ...
    public long[] getLongArray(int index) {
        NbtElement nbtElement;
        if (index >= 0 && index < this.value.size() && (nbtElement = this.value.get(index)).getType() == NbtElement.LONG_ARRAY_TYPE) {
