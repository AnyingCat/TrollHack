...
public class ItemGroups {
    ...
    public static ItemGroup registerAndGetDefault(Registry<ItemGroup> registry) {
        ...
        Registry.register(registry, FUNCTIONAL, ItemGroup.create(ItemGroup.Row.TOP, 3).displayName(Text.translatable("itemGroup.functional")).icon(() -> new ItemStack(Items.OAK_SIGN)).entries((displayContext, entries) -> {
            ...
            displayContext.lookup().getOptionalWrapper(RegistryKeys.PAINTING_VARIANT).ifPresent(registryWrapper -> ItemGroups.addPaintings(entries, displayContext.lookup(), registryWrapper, registryEntry -> registryEntry.isIn(PaintingVariantTags.PLACEABLE), ItemGroup.StackVisibility.PARENT_AND_SEARCH_TABS));
        ...
        Registry.register(registry, COMBAT, ItemGroup.create(ItemGroup.Row.BOTTOM, 1).displayName(Text.translatable("itemGroup.combat")).icon(() -> new ItemStack(Items.NETHERITE_SWORD)).entries((displayContext, entries) -> {
            ...
            displayContext.lookup().getOptionalWrapper(RegistryKeys.POTION).ifPresent(registryWrapper -> ItemGroups.addPotions(entries, registryWrapper, Items.TIPPED_ARROW, ItemGroup.StackVisibility.PARENT_AND_SEARCH_TABS, displayContext.enabledFeatures()));
        }).build());
    ...
    private static void addMaxLevelEnchantedBooks(ItemGroup.Entries entries, RegistryWrapper<Enchantment> registryWrapper, ItemGroup.StackVisibility stackVisibility) {
        registryWrapper.streamEntries().map(enchantmentEntry -> EnchantedBookItem.forEnchantment(new EnchantmentLevelEntry((RegistryEntry<Enchantment>)enchantmentEntry, ((Enchantment)enchantmentEntry.value()).getMaxLevel()))).forEach(stack -> entries.add((ItemStack)stack, stackVisibility));
    }
