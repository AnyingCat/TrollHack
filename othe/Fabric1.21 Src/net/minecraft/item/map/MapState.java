...
extends PersistentState {
    ...
    public static MapState fromNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup) {
        RegistryKey<World> registryKey = DimensionType.worldFromDimensionNbt(new Dynamic<NbtElement>(NbtOps.INSTANCE, nbt.get("dimension"))).resultOrPartial(LOGGER::error).orElseThrow(() -> new IllegalArgumentException("Invalid map dimension: " + String.valueOf(nbt.get("dimension"))));
        ...
        List list = MapBannerMarker.LIST_CODEC.parse(registryOps, nbt.get("banners")).resultOrPartial(banner -> LOGGER.warn("Failed to parse map banner: '{}'", banner)).orElse(List.of());
        for (MapBannerMarker mapBannerMarker : list) {
            mapState.banners.put(mapBannerMarker.getKey(), mapBannerMarker);
            mapState.addDecoration(mapBannerMarker.getDecorationType(), null, mapBannerMarker.getKey(), mapBannerMarker.pos().getX(), mapBannerMarker.pos().getZ(), 180.0, mapBannerMarker.name().orElse(null));
        }
        ...
        for (int k = 0; k < nbtList.size(); ++k) {
            ...
            if (mapFrameMarker == null) continue;
            mapState.frames.put(mapFrameMarker.getKey(), mapFrameMarker);
            mapState.addDecoration(MapDecorationTypes.FRAME, null, MapState.getFrameDecorationKey(mapFrameMarker.getEntityId()), mapFrameMarker.getPos().getX(), mapFrameMarker.getPos().getZ(), mapFrameMarker.getRotation(), null);
        }
        return mapState;
    }
    ...
    public void update(PlayerEntity player, ItemStack stack) {
        ...
        for (int i = 0; i < this.updateTrackers.size(); ++i) {
            ...
            if (stack.isInFrame() || playerUpdateTracker2.player.getWorld().getRegistryKey() != this.dimension || !this.showDecorations) continue;
        ...
        if (stack.isInFrame() && this.showDecorations) {
            ...
            if (mapFrameMarker != null && itemFrameEntity.getId() != mapFrameMarker.getEntityId() && this.frames.containsKey(mapFrameMarker.getKey())) {
                this.removeDecoration(MapState.getFrameDecorationKey(mapFrameMarker.getEntityId()));
            }
            ...
            this.addDecoration(MapDecorationTypes.FRAME, player.getWorld(), MapState.getFrameDecorationKey(itemFrameEntity.getId()), blockPos.getX(), blockPos.getZ(), itemFrameEntity.getHorizontalFacing().getHorizontal() * 90, null);
    ...
    private void addDecoration(RegistryEntry<MapDecorationType> type, @Nullable WorldAccess world, String key, double x, double z, double rotation, @Nullable Text text) {
        ...
        if (!(mapDecoration = new MapDecoration(type, b, c, d, Optional.ofNullable(text))).equals(mapDecoration2 = this.decorations.put(key, mapDecoration))) {
