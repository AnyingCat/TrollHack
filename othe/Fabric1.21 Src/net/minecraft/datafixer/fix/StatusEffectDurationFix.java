...
extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("EffectDurationEntity", schema.getType(TypeReferences.ENTITY), entityTyped -> entityTyped.update(DSL.remainderFinder(), this::fixEntityStatusEffects)), this.fixTypeEverywhereTyped("EffectDurationPlayer", schema.getType(TypeReferences.PLAYER), playerTyped -> playerTyped.update(DSL.remainderFinder(), this::fixEntityStatusEffects)), this.fixTypeEverywhereTyped("EffectDurationItem", type, itemStackTyped -> {
            ...
            if (optional.filter(POTION_ITEM_IDS::contains).isPresent() && (optional2 = itemStackTyped.getOptionalTyped(opticFinder2)).isPresent()) {
                Dynamic<?> dynamic = optional2.get().get(DSL.remainderFinder());
                Typed<?> typed = optional2.get().set(DSL.remainderFinder(), dynamic.update("CustomPotionEffects", this::fixPotionEffects));
