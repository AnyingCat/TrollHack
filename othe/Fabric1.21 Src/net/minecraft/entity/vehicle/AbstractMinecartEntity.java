...
extends VehicleEntity {
    ...
    public static AbstractMinecartEntity create(ServerWorld world, double x, double y, double z, Type type, ItemStack stack, @Nullable PlayerEntity player) {
        AbstractMinecartEntity abstractMinecartEntity = switch (type.ordinal()) {
        ...
        EntityType.copier(world, stack, player).accept((ChestMinecartEntity)abstractMinecartEntity);
    ...
    public Vec3d updatePassengerForDismount(LivingEntity passenger) {
        ...
        for (EntityPose entityPose2 : immutableList) {
            ...
            if (!(e + g <= (h = Dismounting.getCeilingHeight(mutable, j = MathHelper.ceil(e - (double)mutable.getY() + g), pos -> this.getWorld().getBlockState((BlockPos)pos).getCollisionShape(this.getWorld(), (BlockPos)pos))))) continue;
