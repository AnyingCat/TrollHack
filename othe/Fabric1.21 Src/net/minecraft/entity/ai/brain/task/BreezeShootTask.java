...
extends MultiTickTask<BreezeEntity> {
    ...
    protected void keepRunning(ServerWorld serverWorld, BreezeEntity breezeEntity, long l) {
        ...
        if (BreezeShootTask.isFacingTarget(breezeEntity, livingEntity)) {
            ...
            BreezeWindChargeEntity breezeWindChargeEntity = new BreezeWindChargeEntity(breezeEntity, (World)serverWorld);
    ...
    public static boolean isFacingTarget(BreezeEntity breeze, LivingEntity target) {
        ...
        return vec3d.dotProduct(vec3d2 = target.getPos().subtract(breeze.getPos()).normalize()) > 0.5;
    }
