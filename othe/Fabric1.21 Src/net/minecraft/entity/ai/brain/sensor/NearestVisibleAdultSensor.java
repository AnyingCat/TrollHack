...
extends Sensor<PassiveEntity> {
    ...
    private void findNearestVisibleAdult(PassiveEntity entity, LivingTargetCache livingTarget) {
        Optional<PassiveEntity> optional = livingTarget.findFirst(livingEntity -> livingEntity.getType() == entity.getType() && !livingEntity.isBaby()).map(PassiveEntity.class::cast);
