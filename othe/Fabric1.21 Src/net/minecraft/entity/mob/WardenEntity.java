...
implements Vibrations {
    ...
    public void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        WardenAngerManager.createCodec(this::isValidTarget).encodeStart(registryOps, this.angerManager).resultOrPartial(string -> LOGGER.error("Failed to encode anger state for Warden: '{}'", string)).ifPresent(angerNbt -> nbt.put("anger", (NbtElement)angerNbt));
        Vibrations.ListenerData.CODEC.encodeStart(registryOps, this.vibrationListenerData).resultOrPartial(string -> LOGGER.error("Failed to encode vibration listener for Warden: '{}'", string)).ifPresent(listenerData -> nbt.put("listener", (NbtElement)listenerData));
    }
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("anger")) {
            WardenAngerManager.createCodec(this::isValidTarget).parse(registryOps, nbt.get("anger")).resultOrPartial(string -> LOGGER.error("Failed to parse anger state for Warden: '{}'", string)).ifPresent(angerManager -> {
                this.angerManager = angerManager;
            });
            this.updateAnger();
        }
        if (nbt.contains("listener", NbtElement.COMPOUND_TYPE)) {
            Vibrations.ListenerData.CODEC.parse(registryOps, nbt.getCompound("listener")).resultOrPartial(string -> LOGGER.error("Failed to parse vibration listener for Warden: '{}'", string)).ifPresent(listenerData -> {
