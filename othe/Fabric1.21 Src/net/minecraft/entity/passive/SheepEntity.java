...
implements Shearable {
    ...
    private DyeColor getChildColor(AnimalEntity firstParent, AnimalEntity secondParent) {
        ...
        CraftingRecipeInput craftingRecipeInput = SheepEntity.createChildColorRecipeInput(dyeColor, dyeColor2);
        return this.getWorld().getRecipeManager().getFirstMatch(RecipeType.CRAFTING, craftingRecipeInput, this.getWorld()).map(recipe -> ((CraftingRecipe)recipe.value()).craft(craftingRecipeInput, this.getWorld().getRegistryManager())).map(ItemStack::getItem).filter(DyeItem.class::isInstance).map(DyeItem.class::cast).map(DyeItem::getColor).orElseGet(() -> this.getWorld().random.nextBoolean() ? dyeColor : dyeColor2);
    }
