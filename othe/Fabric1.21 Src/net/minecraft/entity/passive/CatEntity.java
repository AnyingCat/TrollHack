    ...
    public CatEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
        ...
        if (catEntity != null && passiveEntity instanceof CatEntity) {
            ...
            if (this.random.nextBoolean()) {
                catEntity.setVariant((RegistryEntry<CatVariant>)this.getVariant());
            } else {
                catEntity.setVariant((RegistryEntry<CatVariant>)catEntity2.getVariant());
            }
            if (this.isTamed()) {
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            if (livingEntity instanceof PlayerEntity) {
                ...
                BlockState blockState = ((World)this.cat.getWorld()).getBlockState(blockPos);
        ...
        private void dropMorningGifts() {
            ...
            LootTable lootTable = ((World)this.cat.getWorld()).getServer().getReloadableRegistries().getLootTable(LootTables.CAT_MORNING_GIFT_GAMEPLAY);
            ...
            for (ItemStack itemStack : list) {
                this.cat.getWorld().spawnEntity(new ItemEntity((World)this.cat.getWorld(), (double)mutable.getX() - (double)MathHelper.sin(this.cat.bodyYaw * ((float)Math.PI / 180)), mutable.getY(), (double)mutable.getZ() + (double)MathHelper.cos(this.cat.bodyYaw * ((float)Math.PI / 180)), itemStack));
            }
        }
