...
implements Attackable {
    ...
    private final Reference2ObjectMap<Enchantment, Set<EnchantmentLocationBasedEffect>> locationBasedEnchantmentEffects = new Reference2ObjectArrayMap<Enchantment, Set<EnchantmentLocationBasedEffect>>();
        ...
        if ((world = this.getWorld()) instanceof ServerWorld) {
            ...
            if (onGround && this.fallDistance > 0.0f) {
                ...
                if ((double)this.fallDistance > d && !state.isAir()) {
                    ...
                    float k = MathHelper.ceil((double)this.fallDistance - d);
    ...
    public void baseTick() {
        ...
        if (this.isAlive()) {
                ...
                if (bl2) {
                    ...
                    if (this.getAir() == -20) {
                        ...
                        for (int i = 0; i < 8; ++i) {
                            ...
                            this.getWorld().addParticle(ParticleTypes.BUBBLE, this.getX() + f, this.getY() + g, this.getZ() + h, ((Vec3d)vec3d).x, ((Vec3d)vec3d).y, ((Vec3d)vec3d).z);
                        }
            ...
            vec3d = this.getWorld();
            if (vec3d instanceof ServerWorld) {
                ServerWorld serverWorld2 = (ServerWorld)vec3d;
                BlockPos blockPos = this.getBlockPos();
    ...
    public float getScale() {
        ...
        return this.clampScale((float)attributeContainer.getValue(EntityAttributes.GENERIC_SCALE));
    }
    ...
    public void onEquipStack(EquipmentSlot slot, ItemStack oldStack, ItemStack newStack) {
        ...
        if (!this.getWorld().isClient() && !this.isSpectator()) {
            if (!this.isSilent() && equipment != null && equipment.getSlotType() == slot) {
                this.getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), equipment.getEquipSound(), this.getSoundCategory(), 1.0f, 1.0f, this.random.nextLong());
            }
    ...
    public void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        if (!this.activeStatusEffects.isEmpty()) {
            ...
            nbt.put(ACTIVE_EFFECTS_NBT_KEY, nbtList);
        }
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("Team", NbtElement.STRING_TYPE)) {
            ...
            boolean bl2 = bl = team != null && scoreboard.addScoreHolderToTeam(this.getUuidAsString(), team);
            if (!bl) {
    ...
    public final boolean tryUseTotem(DamageSource source) {
    ...
    protected void damageEquipment(DamageSource source, float amount, EquipmentSlot ... slots) {
        ...
        for (EquipmentSlot equipmentSlot : slots) {
            ...
            if (!(itemStack.getItem() instanceof ArmorItem) || !itemStack.takesDamageFrom(source)) continue;
    ...
    public void travel(Vec3d movementInput) {
        if (this.isLogicalSideForUpdatingMovement()) {
            ...
            if (this.isTouchingWater() && this.shouldSwimInFluids() && !this.canWalkOnFluid(fluidState)) {
                ...
                double q = vec3d6.y;
                q = this.hasStatusEffect(StatusEffects.LEVITATION) ? (q += (0.05 * (double)(this.getStatusEffect(StatusEffects.LEVITATION).getAmplifier() + 1) - vec3d6.y) * 0.2) : (!this.getWorld().isClient || this.getWorld().isChunkLoaded(blockPos) ? (q -= d) : (this.getY() > (double)this.getWorld().getBottomY() ? -0.1 : 0.0));
                if (this.hasNoDrag()) {
                    this.setVelocity(vec3d6.x, q, vec3d6.z);
                } else {
                    this.setVelocity(vec3d6.x * (double)f, this instanceof Flutterer ? q * (double)f : q * (double)0.98f, vec3d6.z * (double)f);
                }
            }
        }
    ...
    private Map<EquipmentSlot, ItemStack> getEquipmentChanges() {
        ...
        for (EquipmentSlot equipmentSlot : EquipmentSlot.values()) {
            ...
            itemStack.applyAttributeModifiers(equipmentSlot, (attribute, modifier) -> {
                EntityAttributeInstance entityAttributeInstance = attributeContainer.getCustomInstance((RegistryEntry<EntityAttribute>)attribute);
                if (entityAttributeInstance != null) {
    ...
    private void sendEquipmentChanges(Map<EquipmentSlot, ItemStack> equipmentChanges) {
        ...
        equipmentChanges.forEach((slot, stack) -> {
            ...
            switch (slot.getType()) {
                case HAND: {
                    this.setSyncedHandStack((EquipmentSlot)slot, itemStack);
                    break;
                }
                case HUMANOID_ARMOR: {
                    this.setSyncedArmorStack((EquipmentSlot)slot, itemStack);
                    break;
                }
                case ANIMAL_ARMOR: {
    ...
    public void tickMovement() {
        ...
        playerEntity = (PlayerEntity)var11_11;
        if (this.isAlive()) {
    ...
    public boolean isBlocking() {
        ...
        return item.getMaxUseTime(this.activeItemStack, this) - this.itemUseTimeLeft >= 5;
    }
    ...
    public void sleep(BlockPos pos) {
        ...
        if ((blockState = this.getWorld().getBlockState(pos)).getBlock() instanceof BedBlock) {
            this.getWorld().setBlockState(pos, (BlockState)blockState.with(BedBlock.OCCUPIED, true), Block.NOTIFY_ALL);
        }
    ...
    public void wakeUp() {
        this.getSleepingPosition().filter(this.getWorld()::isChunkLoaded).ifPresent(pos -> {
            ...
            if (blockState.getBlock() instanceof BedBlock) {
                ...
                this.getWorld().setBlockState((BlockPos)pos, (BlockState)blockState.with(BedBlock.OCCUPIED, false), Block.NOTIFY_ALL);
    ...
    public final ItemStack tryEatFood(World world, ItemStack stack) {
        ...
        if (foodComponent != null) {
            return this.eatFood(world, stack, foodComponent);
        }
        return stack;
    }
    ...
    private void applyFoodEffects(FoodComponent component) {
        ...
        for (FoodComponent.StatusEffectEntry statusEffectEntry : list) {
            if (!(this.random.nextFloat() < statusEffectEntry.probability())) continue;
    ...
    public EquipmentSlot getPreferredEquipmentSlot(ItemStack stack) {
        ...
        if (equipment != null && this.canUseSlot(equipmentSlot = equipment.getSlotType())) {
    ...
    public boolean isInvulnerableTo(DamageSource damageSource) {
        ...
        World world;
        return super.isInvulnerableTo(damageSource) || (world = this.getWorld()) instanceof ServerWorld && EnchantmentHelper.isInvulnerableTo(serverWorld = (ServerWorld)world, this, damageSource);
    }
