...
public record RegistryPair<T>(Optional<RegistryEntry<T>> entry, RegistryKey<T> key) {
    ...
    public static <T> Codec<RegistryPair<T>> createCodec(RegistryKey<Registry<T>> registryRef, Codec<RegistryEntry<T>> entryCodec) {
        return Codec.either(entryCodec, RegistryKey.createCodec(registryRef).comapFlatMap(registryKey -> DataResult.error(() -> "Cannot parse as key without registry"), Function.identity())).xmap(RegistryPair::create, RegistryPair::asEither);
    }
