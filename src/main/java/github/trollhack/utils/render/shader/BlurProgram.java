...
public class BlurProgram implements Mc {
    ...
    public static final ManagedCoreShader BLUR = ShaderEffectManager.getInstance()
            .manageCoreShader(Identifier.of("troll", "blur"), VertexFormats.POSITION);

    public BlurProgram() {
        setup();
    }

    public void setParameters(float x, float y, float width, float height, float tl, float tr, float br, float bl, Color c1, float blurStrenth, float blurOpacity) {
        if (input == null)
    ...
    protected void setup() {
        ...
        this.radii = BLUR.findUniform4f("radii");
